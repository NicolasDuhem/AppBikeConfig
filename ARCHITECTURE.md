...
Password is checked against `app_users.password_hash`
Session is created
Protected pages require session
JWT stores the current `userId`
Session callback exposes the current user id to server-side helpers
Authorization
...
Save row updates back through API
Enforce role check before write
Write audit log
---
Matrix bulk update flow
User chooses a bulk update action
UI checks permission via `/api/me`
API enforces `sys_admin` or `sales_admin`
Matching rows are updated
Audit row is written to `audit_log`
---
Order flow
User chooses country
App filters rows where availability is true
Cascading filters narrow product selection
Product tiles are displayed
---
Setup flow
User adds or removes setup options
Setup options feed lists and dropdowns elsewhere in the app
API enforces `sys_admin` or `product_admin`
Audit row is written
---
SKU Definition flow
User reads digit rules
User picks options
App builds 30-character SKU
Empty digit positions become `_`
Write actions require `sys_admin` or `product_admin`
---
Bike Builder flow
User selects allowed values per option
App generates cartesian product of selected choices
App builds SKU code from rules
User ticks valid rows
User pushes selected rows into Matrix
API enforces `sys_admin` or `product_admin`
Audit row is written
---
User administration flow
sys_admin logs in
sys_admin opens `/users`
sys_admin creates user with email + initial password
Password is bcrypt-hashed before storing
sys_admin assigns one or multiple roles
sys_admin can deactivate/reactivate a user
...
Update SQL tables or migrations first
Update server-side helpers if permissions change
Update route handler authorization
Update audit logging where new writes are added
Update UI gating from `/api/me`
Update this file
Codex ground rules
...
Pull latest repo
Review changes
Run SQL update script in Neon if needed
Update `.env.local` if new env vars are needed
Run locally:
`npm install`
`npm run dev`
`npm run build`
...
If Vercel is linked, deployment is triggered automatically
Vercel flow
...
Stabilize the auth/RBAC implementation and verify each role end-to-end
Add a seed/bootstrap script for creating the first sys_admin user more easily
Improve README with exact first-user creation steps
Consider adding export/import for Matrix
Consider adding country/region scoped permissions later
Consider future SSO (e.g. Azure AD) if Brompton wants corporate login
---
Summary
