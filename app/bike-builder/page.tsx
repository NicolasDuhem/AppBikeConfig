...
export default function BikeBuilderPage() {
  ...
  function generate() {
    ...
    const result: GeneratedRow[] = rows.map(row => {
      ...
      Object.entries(row).forEach(([optionName, value]) => {
        ...
        if (target && target !== 'ignore') (output[target] as string) = value;
      });
  ...
  return <div className="page">
    <h2>Bike Builder</h2>
    <div className="note">Select the allowed values for each option, generate all potential combinations, tick the exact rows to keep, then push them into the matrix.</div>
    <div className="metricGrid">
      <div className="metric"><div className="metricLabel">Selected groups</div><div className="metricValue">{selectedGroups}</div></div>
      <div className="metric"><div className="metricLabel">Selected options</div><div className="metricValue">{selectedValues}</div></div>
      <div className="metric"><div className="metricLabel">Generated rows</div><div className="metricValue">{generated.length}</div></div>
      <div className="metric"><div className="metricLabel">Ticked</div><div className="metricValue">{ticked}</div></div>
    </div>
